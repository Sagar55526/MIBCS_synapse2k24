...
import { useState, useEffect } from 'react';
import getRemainingTime from '../../utils/countDownTimer'; // Assuming this is a separate file
import CounterAtom from './CounterAtom';
...
const CounterContainer = ({ countDownLimit }) => {
  ...
  useEffect(() => {
    ...
    const updateRemainingTime = () => {
      const newRemainingTime = getRemainingTime(countDownLimit);
      setRemainingTime(newRemainingTime);
    };

    updateRemainingTime(); // Update time on mount
    const timer = setInterval(updateRemainingTime, 1000); // Update every second

    return () => clearInterval(timer); // Clean up timer on unmount
  }, [countDownLimit]); // Dependency array includes countDownLimit
